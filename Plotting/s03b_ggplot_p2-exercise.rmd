...
Bubble plot formula from: https://www.r-graph-gallery.com/320-the-basis-of-bubble-plot.html
```{r, fig.width = 5, fig.height = 2}
gapminder %>% 
...
Shape formula from : https://ggplot2.tidyverse.org/reference/geom_point.html

```{r, fig.width = 5, fig.height = 2}
ggplot(gapminder) +
  geom_point(aes(gdpPercap, lifeExp, shape = "0.1")) +
...
gapminder %>% 
  filter(continent == "Americas") %>% 
  ggplot(aes(country, lifeExp)) + 
  ggridges::geom_density_ridges(lifeExp)
```
...
mtcars %>% 
  ...
  ggplot(aes(cyl)) +
  geom_bar(aes(y = _proportion_, color = transmission))
```
