...
1. Would you say this data is in tidy format?
  ...
  This is not a tidy data because each row does not have one observation; each column does not have one variable and each cell is a value.
...
guest_long %>% 
  group_by(party) %>% 
  summarize(pending_wedding = all(party == "PENDING"))
```
